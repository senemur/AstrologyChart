...
export const calculateSunSign = (day: number, month: number): string => {
  const zodiacDates = [
...
// Moon sign calculation (simplified - uses a lookup based on birth year and month)
export const calculateMoonSign = (day: number, month: number, year: number): string => {
  // Simplified moon sign calculation
  const moonCycle = 29.53;
  const referenceDate = new Date(2000, 0, 6); // Known new moon
  const birthDate = new Date(year, month - 1, day);
  
  const daysDiff = Math.floor((birthDate.getTime() - referenceDate.getTime()) / (1000 * 60 * 60 * 24));
  const moonPhase = daysDiff % moonCycle;
  
  const signs = ["aries", "taurus", "gemini", "cancer", "leo", "virgo", 
                 "libra", "scorpio", "sagittarius", "capricorn", "aquarius", "pisces"];
  
  const signIndex = Math.floor((moonPhase / moonCycle) * 12) % 12;
  return signs[signIndex];
};

// Rising sign calculation (simplified - based on birth hour)
export const calculateRisingSign = (hour: number, month: number): string => {
  const signs = ["aries", "taurus", "gemini", "cancer", "leo", "virgo", 
                 "libra", "scorpio", "sagittarius", "capricorn", "aquarius", "pisces"];
  
  // The rising sign changes approximately every 2 hours
  // Starting sign is based on the sun sign at sunrise
  const sunriseSign = (month - 1) % 12;
  const hourOffset = Math.floor(hour / 2);
  
  const risingIndex = (sunriseSign + hourOffset) % 12;
